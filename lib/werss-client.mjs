...
/** 批量「更新公众号文章」时单次抓取可能很慢（Playwright），与列表/搜索等短请求区分 */
export const BULK_MP_UPDATE_TIMEOUT_MS = 180000;
...
export class WeRSSClient {
   ...
   */
  async updateMp(accessToken, mpId, startPage = 0, endPage = 1, timeoutMs = DEFAULT_TIMEOUT_MS) {
   ...
   * 获取消息任务列表
   */
  async getMessageTasks(accessToken, status = null, limit = 50, offset = 0) {
    const q = new URLSearchParams({ limit: String(limit), offset: String(offset) });
    if (status !== null) q.set('status', String(status));
    const data = await this._request('GET', `/message_tasks?${q.toString()}`, {
      headers: this._authHeaders(accessToken),
    });
    return data?.data ?? data;
  }
   ...
   * 执行单个消息任务
   */
  async runMessageTask(accessToken, taskId, isTest = false) {
    const data = await this._request('GET', `/message_tasks/${encodeURIComponent(taskId)}/run?isTest=${isTest}`, {
      headers: this._authHeaders(accessToken),
      timeout: 60000, // 执行任务可能较慢
    });
    return data?.data ?? data;
  }

  /**
   * 刷新所有公众号：获取全部已启用的公众号，逐个触发文章更新
   * @returns { total, updated, results }
   */
  async refreshAllMps(accessToken) {
    // 获取所有公众号（分页取完）
    const allMps = [];
    let offset = 0;
    const limit = 100;
    while (true) {
      const result = await this.getMpList(accessToken, limit, offset);
    ...
    if (allMps.length === 0) return { total: 0, updated: 0, results: [] };
    ...
    for (const mp of allMps) {
      const mpId = mp.id || mp.mp_id;
      if (!mpId) continue;
      idx++;
      try {
        console.log(`[werss] refreshAllMps ${idx}/${allMps.length} mp_id=${mpId} (${mp.mp_name || ''})`);
        const data = await this.updateMp(accessToken, mpId, 0, 1, BULK_MP_UPDATE_TIMEOUT_MS);
        updated++;
        results.push({ mpId, name: mp.mp_name || '', ok: true, data });
      } catch (e) {
        results.push({ mpId, name: mp.mp_name || '', ok: false, error: e.message });
      }
    }

    return { total: allMps.length, updated, results };
  }
