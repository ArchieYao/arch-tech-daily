...
export function initDefaultAdmin() {
  ...
  if (data.users.length === 0) {
    ...
    console.warn('[auth] ⚠️  请尽快登录后台修改默认密码！（设置 → 修改密码）');
...
/** 与 Express req.path 一致：无 query；去掉末尾 / 便于比较 */
...
function isAdminApi(path) {
  ...
  // 须先于 /api/werss/ 前缀判断：否则 open-ui 会被当成管理 API
  if (p === '/api/werss/open-ui') return false;
  for (const prefix of ADMIN_API_PREFIXES) {
    if (p.startsWith(prefix)) return true;
  }
  if (p === '/api/rss-sources') return true;
  return false;
}

export function authMiddleware(req, res, next) {
  const p = normalizeApiPath(req.path);
  if (p === '/api/auth/login' || p === '/api/auth/status') return next();
  // 公开：跳转 we-mp-rss 管理页（302）
  if (p === '/api/werss/open-ui') return next();
